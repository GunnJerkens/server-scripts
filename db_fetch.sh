...
if [ -z "$remote_ssh" ]; then
  ...
  ssh -p $remote_ssh_port $remote_ssh \ "mysqldump --no-tablespaces -u$remote_username -p'$remote_password' $remote_database" \ | mysql -u$env_username -p$env_password $env_database
fi
